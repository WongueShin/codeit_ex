...
class CustomerServiceCenter:
    ...
    def process_complaint(self):
        ...
        if len(self.queue) == 0:
            ...
            comp = self.queue.popleft()
            print(f'{comp.name}님의 {comp.content} 문의 내용 접수 되었습니다. 담당자가 배정되면 {comp.email}로 연락드리겠습니다!')

    def add_complaint(self, name, email, content):
        """새로운 문의를 큐에 추가 시켜주는 메소드"""
        # 코드를 쓰세요
        new_comp = CustomerComplaint(name, email, content)
        self.queue.append(new_comp)
